
-- R01
INSERT INTO "administrator" (id,username,"password") VALUES (1,'Amanda','QHA35ZTP5TI');
INSERT INTO "administrator" (id,username,"password") VALUES (2,'Kiona','NID14NVH1CP');
INSERT INTO "administrator" (id,username,"password") VALUES (3,'Shaeleigh','XBR72IRG1JG');
INSERT INTO "administrator" (id,username,"password") VALUES (4,'Ina','XLA52GNN6UN');

-- R02
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (1,'Helen','Cooper Ballard','ultrices@dolor.com','NWR47LXV6QK','Tomsk','Female',53,'www.blue.com','BLOB','nunc nulla vulputate dui, nec tempus mauris','false');
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (2,'August','Derek Maynard','sociis@nuncnullavulputate.net','UXT14OJC5PT','Pierrefonds','Other',77,'www.yellow.com','BLOB','Maecenas libero est, congue a, aliquet','true');
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (3,'Audra','Wynter Blake','sed@at.net','ANU77EMG9HG','Ramagundam','Male',63,'www.yellow.com','BLOB','interdum','false');
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (4,'Martina','Forrest Gallegos','et.libero.Proin@famesacturpis.com','QIO48UFM1ZT','Polino','Male',20,'www.green.com','BLOB','Ut tincidunt orci quis lectus. Nullam','false');
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (5,'Brock','Sebastian Owens','nisi@tinciduntnibhPhasellus.edu','HPI41NEF1AK','Liberia','Female',85,'www.yellow.com','BLOB','et,','true');
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (6,'Dillon','Phillip Long','adipiscing@gravidanon.net','FSE56FKZ6MD','Uberaba','Other',49,'www.red.com','BLOB','Aenean euismod mauris','true');
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (7,'Leilani','Clementine Barlow','arcu.Nunc@maurisMorbi.co.uk','LKN43VGT0RT','Mosquera','Other',61,'www.yellow.com','BLOB','mus. Donec dignissim magna a tortor.','false');
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (8,'Hunter','Leila Chapman','gravida@tristiquealiquet.co.uk','WLI54WBF5ED','Penrith','Male',43,'www.red.com','BLOB','sem ut cursus luctus, ipsum leo elementum','true');
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (9,'Bevis','Dennis Robinson','nec@pedeet.net','COU01HYC1TC','Anantapur','Male',94,'www.blue.com','BLOB','dapibus ligula. Aliquam erat','false');
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (10,'Raymond','Lionel David','pharetra.felis.eget@aenim.com','PEH11CVX6KP','Massa e Cozzile','Other',60,'www.red.com','BLOB','ligula. Aliquam erat volutpat. Nulla dignissim.','false');
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (11,'Cole','Lee Hamilton','pede.nonummy@enimdiamvel.co.uk','CWY53UUR1ZN','Isla de Pascua','Other',41,'www.green.com','BLOB','lacus pede sagittis augue, eu tempor','true');
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (12,'Tarik','Indira Armstrong','Sed.eu.nibh@cursusvestibulum.edu','DSI59MMZ5IG','Goulburn','Other',94,'www.blue.com','BLOB','ipsum ac mi eleifend','true');
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (13,'Yen','Sybil Vazquez','Vestibulum@maurissagittis.edu','KPB80GZG1YY','Tobermory','Other',31,'www.yellow.com','BLOB','pellentesque. Sed dictum. Proin','false');
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (14,'Lacey','Seth Haynes','elementum.sem@velitSedmalesuada.org','KCR79FYD7ME','Narbonne','Male',32,'www.yellow.com','BLOB','nibh. Phasellus','true');
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (15,'Jillian','Daryl Conway','mus.Proin@urnajustofaucibus.co.uk','BGP44UUY7LU','Genzano di Lucania','Male',41,'www.green.com','BLOB','sagittis','true');
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (16,'Jakeem','Mason Mccoy','Aenean.egestas.hendrerit@accumsan.edu','LPW39ZEV4PF','Los Vilos','Other',74,'www.red.com','BLOB','vulputate, risus a ultricies adipiscing, enim mi tempor','true');
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (17,'Giacomo','Quail Obrien','Quisque@CuraeDonec.net','UNB21TAK7BE','Naro','Other',84,'www.red.com','BLOB','elementum at, egestas a, scelerisque sed,','false');
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (18,'Octavius','Alyssa Cash','lorem@tristique.com','QOU69UKR3WH','Dornbirn','Male',78,'www.red.com','BLOB','tincidunt adipiscing.','true');
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (19,'Reed','Jacqueline Abbott','non@hendreritDonecporttitor.org','HSU59LPW8ZF','Pelarco','Other',60,'www.red.com','BLOB','mauris. Integer','true');
INSERT INTO "user" (id,username,"name",email,"password","address",gender,age,website,picture,"description",active) VALUES (20,'Stone','Sloane Saunders','ac@placerat.co.uk','VHI14PIN1WY','Yeotmal','Male',77,'www.red.com','BLOB','ac ipsum. Phasellus vitae mauris sit','false');

-- R03
INSERT INTO "banned_user" (id_user,since,reason) VALUES (5,'2020-03-19T17:03:02-07:00','lectus justo eu arcu. Morbi sit amet massa. Quisque porttitor');
INSERT INTO "banned_user" (id_user,since,reason) VALUES (14,'2020-05-05T02:21:21-07:00','eget metus. In nec orci. Donec nibh. Quisque nonummy ipsum');
INSERT INTO "banned_user" (id_user,since,reason) VALUES (4,'2020-05-19T14:40:31-07:00','sed dolor. Fusce mi lorem, vehicula et, rutrum eu, ultrices');
INSERT INTO "banned_user" (id_user,since,reason) VALUES (10,'2020-11-08T21:05:37-08:00','tincidunt aliquam arcu. Aliquam ultrices iaculis odio. Nam interdum enim');

-- R04
INSERT INTO "suspension" (id,id_user,"from",until,reason) VALUES (1,1,'2021-02-23T15:20:30-08:00','2022-08-12T07:03:23-07:00','sed, est. Nunc');
INSERT INTO "suspension" (id,id_user,"from",until,reason) VALUES (2,9,'2021-03-20T15:09:43-07:00','2021-06-17T14:25:34-07:00','et arcu imperdiet ullamcorper.');
INSERT INTO "suspension" (id,id_user,"from",until,reason) VALUES (3,18,'2020-12-10T00:27:52-08:00','2021-01-25T08:16:16-08:00','cursus in, hendrerit consectetuer, cursus et, magna.');
INSERT INTO "suspension" (id,id_user,"from",until,reason) VALUES (4,4,'2020-01-06T18:37:03-08:00','2020-04-04T13:44:50-07:00','Quisque tincidunt');
INSERT INTO "suspension" (id,id_user,"from",until,reason) VALUES (5,3,'2020-04-29T05:07:02-07:00','2021-04-04T13:53:18-07:00','eu nulla at');
INSERT INTO "suspension" (id,id_user,"from",until,reason) VALUES (6,7,'2020-05-02T20:48:31-07:00','2020-07-29T11:26:22-07:00','nunc sed pede. Cum sociis natoque penatibus et');
INSERT INTO "suspension" (id,id_user,"from",until,reason) VALUES (7,18,'2021-09-18T17:43:43-07:00','2022-03-02T22:43:28-08:00','neque sed dictum eleifend,');

-- R05
INSERT INTO "category" (id,"name") VALUES (1,'Board Games');
INSERT INTO "category" (id,"name") VALUES (2,'Video Games');
INSERT INTO "category" (id,"name") VALUES (3,'Card Games');
INSERT INTO "category" (id,"name") VALUES (4,'Role-Playing Games');

-- R06
INSERT INTO "event" (id,id_organizer,title,visibility,"description",picture,"start_date",end_date,"type","location",max_attendance,cancelled,keywords,id_category,win_points,draw_points,loss_points,leaderboard) VALUES (1,2,'faucibus ut, nulla. Cras',      'Private', 'Nullam scelerisque neque sed sem egestas blandit. Nam nulla magna,','null','2021-03-10T18:32:20-08:00','2021-04-30T03:54:50-07:00','Virtual','Subbiano',55,'true','null',2,93,91,38,'true');
INSERT INTO "event" (id,id_organizer,title,visibility,"description",picture,"start_date",end_date,"type","location",max_attendance,cancelled,keywords,id_category,win_points,draw_points,loss_points,leaderboard) VALUES (2,4,'nascetur ridiculus mus. Donec', 'Public',  'Nunc pulvinar arcu et pede. Nunc sed orci lobortis augue scelerisque mollis. Phasellus libero','null','2021-03-15T04:35:58-07:00','2021-04-29T10:53:13-07:00','Mixed','Borgone Susa',31,'false','null',1,19,5,13,'false');
INSERT INTO "event" (id,id_organizer,title,visibility,"description",picture,"start_date",end_date,"type","location",max_attendance,cancelled,keywords,id_category,win_points,draw_points,loss_points,leaderboard) VALUES (3,6,'diam lorem, auctor quis,',      'Public',  'cursus, diam at pretium aliquet, metus urna convallis erat, eget tincidunt dui','null','2021-03-18T07:21:28-07:00','2021-04-26T15:01:28-07:00','Mixed','Banjarmasin',79,'true','null',2,8,90,39,'true');
INSERT INTO "event" (id,id_organizer,title,visibility,"description",picture,"start_date",end_date,"type","location",max_attendance,cancelled,keywords,id_category,win_points,draw_points,loss_points,leaderboard) VALUES (4,8,'pharetra ut, pharetra sed,',    'Public',  'lorem, vehicula et, rutrum eu, ultrices sit amet, risus. Donec nibh enim, gravida sit amet, dapibus id, blandit at,','null','2021-03-12T08:21:36-08:00','2021-04-26T05:01:44-07:00','Mixed','Whitehorse',82,'true','null',4,36,94,99,'true');
INSERT INTO "event" (id,id_organizer,title,visibility,"description",picture,"start_date",end_date,"type","location",max_attendance,cancelled,keywords,id_category,win_points,draw_points,loss_points,leaderboard) VALUES (5,10,'augue. Sed molestie. Sed',      'Public',  'Donec tincidunt. Donec vitae erat vel pede blandit congue. In scelerisque scelerisque dui. Suspendisse ac metus vitae velit egestas lacinia.','null','2021-03-17T21:44:52-07:00','2021-04-27T04:35:00-07:00','Mixed','100 Mile House',87,'false','null',1,30,69,92,'true');
INSERT INTO "event" (id,id_organizer,title,visibility,"description",picture,"start_date",end_date,"type","location",max_attendance,cancelled,keywords,id_category,win_points,draw_points,loss_points,leaderboard) VALUES (6,12,'tempus, lorem fringilla ornare','Private', 'ullamcorper eu, euismod ac, fermentum vel, mauris. Integer sem elit, pharetra ut, pharetra sed,','null','2021-03-11T07:02:25-08:00','2021-04-27T08:21:29-07:00','Virtual','B.S.D.',56,'true','null',2,88,3,92,'true');
INSERT INTO "event" (id,id_organizer,title,visibility,"description",picture,"start_date",end_date,"type","location",max_attendance,cancelled,keywords,id_category,win_points,draw_points,loss_points,leaderboard) VALUES (7,14,'dolor. Fusce feugiat. Lorem',   'Private', 'turpis vitae purus gravida sagittis. Duis gravida. Praesent eu nulla at','null','2021-03-11T22:51:12-08:00','2021-04-27T16:30:48-07:00','Virtual','Butzbach',63,'false','null',3,12,91,20,'true');
INSERT INTO "event" (id,id_organizer,title,visibility,"description",picture,"start_date",end_date,"type","location",max_attendance,cancelled,keywords,id_category,win_points,draw_points,loss_points,leaderboard) VALUES (8,16,'ullamcorper, nisl arcu iaculis','Public',  'Nulla tincidunt, neque vitae semper egestas, urna justo faucibus lectus, a sollicitudin orci sem eget','null','2021-03-19T10:40:17-07:00','2021-04-27T15:56:12-07:00','Virtual','Abingdon',41,'true','null',4,95,88,81,'false');
INSERT INTO "event" (id,id_organizer,title,visibility,"description",picture,"start_date",end_date,"type","location",max_attendance,cancelled,keywords,id_category,win_points,draw_points,loss_points,leaderboard) VALUES (9,18,'id ante dictum cursus.',        'Public',  'litora torquent per conubia nostra, per inceptos hymenaeos. Mauris ut quam vel sapien imperdiet ornare. In faucibus.','null','2021-03-16T01:49:26-07:00','2021-04-29T13:46:14-07:00','Virtual','Yeovil',53,'true','null',2,50,82,9,'false');
INSERT INTO "event" (id,id_organizer,title,visibility,"description",picture,"start_date",end_date,"type","location",max_attendance,cancelled,keywords,id_category,win_points,draw_points,loss_points,leaderboard) VALUES (10,20,'Nullam ut nisi a',            'Public',  'Phasellus dapibus quam quis diam. Pellentesque habitant morbi tristique senectus et','null','2021-03-17T23:57:52-07:00','2021-04-29T10:24:03-07:00','Virtual','Avise',89,'true','null',3,96,96,61,'true');

-- R07
INSERT INTO "poll" (id,id_event,question) VALUES (1,4,'ut dolor dapibus gravida. Aliquam tincidunt, nunc');
INSERT INTO "poll" (id,id_event,question) VALUES (2,8,'vel sapien imperdiet');
INSERT INTO "poll" (id,id_event,question) VALUES (3,7,'orci sem eget massa. Suspendisse eleifend. Cras');
INSERT INTO "poll" (id,id_event,question) VALUES (4,4,'imperdiet non, vestibulum nec, euismod in, dolor. Fusce');
INSERT INTO "poll" (id,id_event,question) VALUES (5,3,'rutrum, justo. Praesent');

-- R08
INSERT INTO "poll_option" (id,id_poll,"option") VALUES (1,1,'velit. Quisque varius.');
INSERT INTO "poll_option" (id,id_poll,"option") VALUES (2,1,'mauris id');
INSERT INTO "poll_option" (id,id_poll,"option") VALUES (3,2,'sociis');
INSERT INTO "poll_option" (id,id_poll,"option") VALUES (4,2,'molestie tellus. Aenean egestas');
INSERT INTO "poll_option" (id,id_poll,"option") VALUES (5,3,'massa. Mauris');
INSERT INTO "poll_option" (id,id_poll,"option") VALUES (6,3,'Cum sociis natoque penatibus');
INSERT INTO "poll_option" (id,id_poll,"option") VALUES (7,3,'dolor quam, elementum at,');
INSERT INTO "poll_option" (id,id_poll,"option") VALUES (8,4,'nonummy. Fusce fermentum');
INSERT INTO "poll_option" (id,id_poll,"option") VALUES (9,4,'placerat eget,');
INSERT INTO "poll_option" (id,id_poll,"option") VALUES (10,4,'posuere vulputate,');
INSERT INTO "poll_option" (id,id_poll,"option") VALUES (11,5,'egestas. Aliquam nec');
INSERT INTO "poll_option" (id,id_poll,"option") VALUES (12,5,'consectetuer rhoncus.');
INSERT INTO "poll_option" (id,id_poll,"option") VALUES (13,5,'malesuada');
INSERT INTO "poll_option" (id,id_poll,"option") VALUES (14,5,'adipiscing');
INSERT INTO "poll_option" (id,id_poll,"option") VALUES (15,5,'neque. Morbi quis urna.');

-- R09
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (9,4,2);
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (16,2,9);
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (8,4,11);
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (16,2,3);
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (3,1,15);
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (5,3,15);
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (6,1,2);
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (14,4,13);
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (17,5,7);
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (11,4,12);
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (15,1,14);
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (9,1,10);
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (20,4,6);
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (4,1,2);
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (11,1,4);
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (5,5,4);
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (18,2,3);
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (1,2,10);
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (9,2,12);
INSERT INTO "poll_answer" (id_user,id_poll,id_poll_option) VALUES (15,1,1);

-- R10
INSERT INTO "file" (id,id_event,"name","data",date_uploaded) VALUES (1,2,'mollis', E'\\x3bfe931080c060f2','2021-03-29T22:06:41-07:00');
INSERT INTO "file" (id,id_event,"name","data",date_uploaded) VALUES (2,3,'amet',E'\\xd4f02fba4b290ba1','2021-08-09T12:45:57-07:00');
INSERT INTO "file" (id,id_event,"name","data",date_uploaded) VALUES (3,4,'lectus',E'\\x53d694fbfb2884e7','2020-10-26T17:40:29-07:00');
INSERT INTO "file" (id,id_event,"name","data",date_uploaded) VALUES (4,9,'semper',E'\\x357db30563ce96e3','2022-01-06T20:07:07-08:00');
INSERT INTO "file" (id,id_event,"name","data",date_uploaded) VALUES (5,6,'pede.',E'\\x2f33849f25d4588b','2020-10-28T21:24:29-07:00');
INSERT INTO "file" (id,id_event,"name","data",date_uploaded) VALUES (6,5,'nec,',E'\\x0ae0a640b28561e3','2021-10-11T21:22:31-07:00');
INSERT INTO "file" (id,id_event,"name","data",date_uploaded) VALUES (7,1,'tincidunt.',E'\\x49bf811eccb4f7d2','2021-04-07T23:54:04-07:00');
INSERT INTO "file" (id,id_event,"name","data",date_uploaded) VALUES (8,3,'orci,',E'\\x35e362259924f533','2021-12-13T01:24:59-08:00');
INSERT INTO "file" (id,id_event,"name","data",date_uploaded) VALUES (9,10,'gravida',E'\\x7d5ac1fce1580c85','2021-03-06T05:49:03-08:00');

-- R11
INSERT INTO "competitor" (id,id_event,"name") VALUES (1,1,'Yen Alvarado');
INSERT INTO "competitor" (id,id_event,"name") VALUES (2,1,'Kellie Alford');
INSERT INTO "competitor" (id,id_event,"name") VALUES (3,1,'Ora Oconnor');
INSERT INTO "competitor" (id,id_event,"name") VALUES (4,2,'Donovan Downs');
INSERT INTO "competitor" (id,id_event,"name") VALUES (5,2,'Lois Frederick');
INSERT INTO "competitor" (id,id_event,"name") VALUES (6,2,'Buckminster Frank');
INSERT INTO "competitor" (id,id_event,"name") VALUES (7,3,'Aquila Quinn');
INSERT INTO "competitor" (id,id_event,"name") VALUES (8,3,'Herman Henson');
INSERT INTO "competitor" (id,id_event,"name") VALUES (9,3,'Clayton Bird');
INSERT INTO "competitor" (id,id_event,"name") VALUES (10,4,'Holmes Mullen');
INSERT INTO "competitor" (id,id_event,"name") VALUES (11,4,'Alisa Rivera');
INSERT INTO "competitor" (id,id_event,"name") VALUES (12,4,'Ginger Yates');
INSERT INTO "competitor" (id,id_event,"name") VALUES (13,5,'Hedy Pittman');
INSERT INTO "competitor" (id,id_event,"name") VALUES (14,5,'Tatiana Skinner');
INSERT INTO "competitor" (id,id_event,"name") VALUES (15,5,'Brittany Wilkerson');
INSERT INTO "competitor" (id,id_event,"name") VALUES (16,6,'Whoopi Berry');
INSERT INTO "competitor" (id,id_event,"name") VALUES (17,6,'Kyle Casey');
INSERT INTO "competitor" (id,id_event,"name") VALUES (18,6,'Elvis Anderson');
INSERT INTO "competitor" (id,id_event,"name") VALUES (19,7,'Gary Strong');
INSERT INTO "competitor" (id,id_event,"name") VALUES (20,7,'Zelenia Baldwin');
INSERT INTO "competitor" (id,id_event,"name") VALUES (21,7,'Raja Morgan');
INSERT INTO "competitor" (id,id_event,"name") VALUES (22,8,'Ruby Woodard');
INSERT INTO "competitor" (id,id_event,"name") VALUES (23,8,'Nathaniel Kerr');
INSERT INTO "competitor" (id,id_event,"name") VALUES (24,8,'Nevada Hurst');
INSERT INTO "competitor" (id,id_event,"name") VALUES (25,9,'Inga Francis');
INSERT INTO "competitor" (id,id_event,"name") VALUES (26,9,'Zelenia Herring');
INSERT INTO "competitor" (id,id_event,"name") VALUES (27,9,'Jermaine Flowers');
INSERT INTO "competitor" (id,id_event,"name") VALUES (28,10,'Merritt Simpson');
INSERT INTO "competitor" (id,id_event,"name") VALUES (29,10,'Cody Ryan');
INSERT INTO "competitor" (id,id_event,"name") VALUES (30,10,'Kevyn Massey');

-- R12
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (1,1,'2021-03-15T18:32:20-08:00','amet, dapibus id,','Winner2',1,2);
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (2,1,'2021-04-01T23:06:28-09:00','convallis est, vitae sodales nisi magna sed dui.','TBD',2,3);
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (3,2,'2021-03-20T04:35:58-05:00','molestie dapibus ligula. Aliquam','Tie',4,5);
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (4,2,'2021-03-25T04:35:58-06:00','Duis','Winner1',4,6);
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (5,3,'2021-03-20T07:21:28-07:00','mauris ipsum porta elit, a feugiat','Tie',7,8);
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (6,3,'2021-03-28T07:21:28-01:00','et malesuada fames ac','Winner2',8,9);
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (7,4,'2021-04-15T08:21:36-08:00','elit fermentum risus, at','TBD',10,12);
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (8,4,'2021-04-20T08:21:36-08:00','nunc','TBD',11,12);
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (9,5,'2021-03-17T21:44:52-07:00','Donec sollicitudin','Winner1',13,14);
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (10,5,'2021-04-01T21:44:52-07:00','consequat auctor, nunc nulla vulputate dui, nec','TBD',14,15);
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (11,6,'2021-03-13T07:02:25-08:00','lectus pede et risus.','Winner2',16,17);
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (12,6,'2021-04-21T07:02:25-08:00','primis in faucibus orci','TBD',17,18);
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (13,7,'2021-03-15T22:51:12-08:00','eu tellus. Phasellus','Tie',19,20);
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (14,7,'2021-04-25T22:51:12-08:00','vulputate, lacus. Cras interdum. Nunc sollicitudin commodo ipsum.','TBD',21,20);
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (15,8,'2021-03-19T22:51:12-08:00','Morbi non sapien molestie orci tincidunt adipiscing.','Tie',22,23);
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (16,8,'2021-04-02T22:51:12-08:00','metus facilisis lorem tristique','TBD',24,23);
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (17,9,'2021-03-20T01:49:26-07:00','tincidunt','Winner1',25,26);
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (18,9,'2021-03-25T01:49:26-07:00','vitae risus. Duis','Winner1',26,27);
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (19,10,'2021-03-29T23:57:52-07:00','ac mattis velit justo','Winner2',29,28);
INSERT INTO "match" (id,id_event,"date","description",result,id_competitor1,id_competitor2) VALUES (20,10,'2021-04-17T23:57:52-07:00','non arcu. Vivamus sit','TBD',29,30);

-- R13
INSERT INTO "comment" (id,id_author,id_event,id_parent,"text","date") VALUES (1,13,7,NULL,'enim. Etiam imperdiet dictum magna. Ut tincidunt orci quis lectus. Nullam suscipit,','2021-02-01T01:22:24-08:00');
INSERT INTO "comment" (id,id_author,id_event,id_parent,"text","date") VALUES (2,2,8,NULL,'vehicula aliquet libero. Integer in magna. Phasellus','2020-09-12T14:13:42-07:00');
INSERT INTO "comment" (id,id_author,id_event,id_parent,"text","date") VALUES (3,2,6,NULL,'at, iaculis quis, pede. Praesent eu dui. Cum sociis natoque','2020-10-20T16:06:10-07:00');
INSERT INTO "comment" (id,id_author,id_event,id_parent,"text","date") VALUES (4,9,7,1,'magna. Ut tincidunt orci','2021-07-31T11:20:41-07:00');
INSERT INTO "comment" (id,id_author,id_event,id_parent,"text","date") VALUES (5,14,1,NULL,'mauris ipsum porta elit, a feugiat tellus lorem eu metus. In lorem. Donec elementum,','2020-06-03T00:42:52-07:00');
INSERT INTO "comment" (id,id_author,id_event,id_parent,"text","date") VALUES (6,4,7,NULL,'Cras sed leo.','2020-06-17T15:44:30-07:00');
INSERT INTO "comment" (id,id_author,id_event,id_parent,"text","date") VALUES (7,4,3,NULL,'tortor. Integer aliquam adipiscing lacus. Ut','2022-02-13T02:43:19-08:00');
INSERT INTO "comment" (id,id_author,id_event,id_parent,"text","date") VALUES (8,5,3,7,'nec, leo. Morbi neque tellus, imperdiet non, vestibulum nec, euismod in, dolor. Fusce feugiat. Lorem ipsum dolor sit amet, consectetuer','2021-06-30T18:51:00-07:00');
INSERT INTO "comment" (id,id_author,id_event,id_parent,"text","date") VALUES (9,18,1,NULL,'commodo ipsum. Suspendisse non leo. Vivamus nibh dolor, nonummy ac, feugiat non, lobortis quis, pede. Suspendisse dui. Fusce diam nunc,','2020-08-01T16:01:28-07:00');
INSERT INTO "comment" (id,id_author,id_event,id_parent,"text","date") VALUES (10,5,6,NULL,'risus odio, auctor vitae, aliquet nec, imperdiet nec, leo. Morbi neque tellus, imperdiet non, vestibulum nec,','2022-03-24T19:39:49-07:00');
INSERT INTO "comment" (id,id_author,id_event,id_parent,"text","date") VALUES (11,14,3,8,'libero lacus, varius et, euismod et, commodo','2022-01-19T16:45:43-08:00');
INSERT INTO "comment" (id,id_author,id_event,id_parent,"text","date") VALUES (12,9,4,NULL,'mauris elit, dictum eu, eleifend','2020-10-18T10:58:45-07:00');
INSERT INTO "comment" (id,id_author,id_event,id_parent,"text","date") VALUES (13,2,7,6,'dignissim tempor arcu. Vestibulum ut eros non enim commodo hendrerit. Donec porttitor tellus non magna. Nam ligula elit,','2021-08-18T01:06:05-07:00');
INSERT INTO "comment" (id,id_author,id_event,id_parent,"text","date") VALUES (14,1,10,NULL,'consectetuer, cursus et, magna. Praesent interdum ligula','2020-07-27T16:49:56-07:00');
INSERT INTO "comment" (id,id_author,id_event,id_parent,"text","date") VALUES (15,10,6,NULL,'id, blandit','2020-12-13T16:34:01-08:00');

-- R14
INSERT INTO "participation" (id_user,id_event,"status") VALUES (20,7,'Declined');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (19,2,'Invitation');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (12,1,'JoinRequest');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (16,10,'Declined');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (7,5,'Invitation');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (16,7,'Accepted');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (18,7,'JoinRequest');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (3,7,'Accepted');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (10,3,'Accepted');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (12,1,'Accepted');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (9,4,'JoinRequest');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (14,1,'Declined');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (5,2,'Declined');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (1,5,'Invitation');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (12,7,'Accepted');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (19,6,'Invitation');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (3,1,'JoinRequest');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (2,5,'Declined');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (6,8,'JoinRequest');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (19,9,'Invitation');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (16,2,'Declined');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (10,1,'Declined');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (13,2,'JoinRequest');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (6,8,'Declined');
INSERT INTO "participation" (id_user,id_event,"status") VALUES (11,10,'Accepted');

-- R15
INSERT INTO "tag" (id,"name") VALUES (1,'at');
INSERT INTO "tag" (id,"name") VALUES (2,'eget');
INSERT INTO "tag" (id,"name") VALUES (3,'aliquet');
INSERT INTO "tag" (id,"name") VALUES (4,'nascetur');
INSERT INTO "tag" (id,"name") VALUES (5,'ac');
INSERT INTO "tag" (id,"name") VALUES (6,'scelerisque');
INSERT INTO "tag" (id,"name") VALUES (7,'ridiculus');
INSERT INTO "tag" (id,"name") VALUES (8,'neque');
INSERT INTO "tag" (id,"name") VALUES (9,'velit');
INSERT INTO "tag" (id,"name") VALUES (10,'mauris');
INSERT INTO "tag" (id,"name") VALUES (11,'cursus');
INSERT INTO "tag" (id,"name") VALUES (12,'sed');
INSERT INTO "tag" (id,"name") VALUES (13,'suspendisse');
INSERT INTO "tag" (id,"name") VALUES (14,'pellentesque');
INSERT INTO "tag" (id,"name") VALUES (15,'malesuada');

-- R16
INSERT INTO "event_tag" (id_user,id_tag) VALUES (6,13);
INSERT INTO "event_tag" (id_user,id_tag) VALUES (10,12);
INSERT INTO "event_tag" (id_user,id_tag) VALUES (1,10);
INSERT INTO "event_tag" (id_user,id_tag) VALUES (4,6);
INSERT INTO "event_tag" (id_user,id_tag) VALUES (9,8);
INSERT INTO "event_tag" (id_user,id_tag) VALUES (7,3);
INSERT INTO "event_tag" (id_user,id_tag) VALUES (8,11);
INSERT INTO "event_tag" (id_user,id_tag) VALUES (10,10);
INSERT INTO "event_tag" (id_user,id_tag) VALUES (9,7);
INSERT INTO "event_tag" (id_user,id_tag) VALUES (2,1);
INSERT INTO "event_tag" (id_user,id_tag) VALUES (3,14);
INSERT INTO "event_tag" (id_user,id_tag) VALUES (7,9);
